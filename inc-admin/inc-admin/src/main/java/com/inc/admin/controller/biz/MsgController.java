...
public class MsgController extends BaseController {
    ...
    public R listByPage(@RequestBody Msg req) {
        req.setUserId(FilterContextHandler.getUserID4Int());
        return R.ok().put("page", msgService.listByPage(req));
    }
     ...
     */
    @PostMapping("/insert")
    public R insert(@RequestBody Msg req) {
        return R.operate(msgService.insert(req)>0);
    }
     ...
     */
    @PostMapping("/update")
    public R update(@RequestBody Msg req) {
        return R.operate(msgService.update(req)>0);
    }
    ...
    public R delete(@Validated @NotNull(message = "编号不能为空") @RequestParam("id") @RequestBody Integer id) {
        return R.operate(msgService.delete(id)>0);
    }
}